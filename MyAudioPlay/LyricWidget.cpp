...
bool LyricWidget::process(QString lyrics)
{    
...
//    lyricFile.close();
    ...
    if (pos == -1) {
        ...
        do {
            ...
            pos = rx.indexIn(lyrics, lastPos);
            if (pos == -1) {
                QString text = lyrics.mid(lastPos).trimmed();
                foreach (const int& time, timeLabels)
                    lines.push_back(LyricLine(time, text));
                break;
            }
            // 步骤3
            QString text = lyrics.mid(lastPos, pos - lastPos);
            if (!text.isEmpty()) {
                foreach (const int& time, timeLabels)
